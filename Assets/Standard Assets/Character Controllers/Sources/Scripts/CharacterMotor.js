...
private function ApplyGravityAndJumping (velocity : Vector3) {
	...
	if (grounded)
	{
		...
		velocity.y *= jumping.speedUpOnJump;
	}
	else {
...
function OnControllerColliderHit (hit : ControllerColliderHit) {
	if (hit.normal.y > 0 && hit.normal.y > groundNormal.y && hit.moveDirection.y < 0) {
		...
		var angle = Mathf.Rad2Deg * Mathf.Acos( Vector3.Dot( hit.normal, movement.velocity.normalized ) );
		if( angle != 180 )
		{
			movement.velocity = Quaternion.Euler(0, ( angle - 90 ), 0) * movement.velocity;
			var timepass = Quaternion.Euler( 0, -90, 0 ) * hit.normal;
		}
	}
}
